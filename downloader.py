...
class HTTPDownloader(BaseDownloader):
        ...
        try:
            ...
            for attempt in range(self.number_of_threads*2):
                ...
                for i in range(self.number_of_threads):
                    ...
                    job = pool.apply_async(self.thread_downloader, (start, end, offset, self.headers, i, q,))
                    jobs.append(job)
    ...
    def thread_downloader(self, start, end, offset, headers, number, q):
        # This try/except is critical for ctrl + c to work on windows.
        try:
            headers['Range'] = 'bytes=%d-%d' % (start+offset, end)

            # specify the starting and ending of the file
            with session.get(self.url, headers=headers) as r:
                try:
                    r.raise_for_status()
                except:
                    return
        ...
        with open(self.path, 'r+b') as f:
            with open(self.partfile, "w+") as metadata:
                ...
                for i in range(self.number_of_threads):
                    ...
                    self.headers['Range'] = 'bytes=%d-%d' % (start+offset, end)

                    with session.get(self.url, headers=self.headers, stream=True) as r:
